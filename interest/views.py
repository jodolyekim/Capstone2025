...
class GPTKeywordSaveView(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            # ✅ Profile.interests 동기화
            profile, _ = Profile.objects.get_or_create(user=user)
            all_interest_ids = Interest.objects.filter(user=user).values_list('id', flat=True)
            profile.interests.set(all_interest_ids)
            profile.save()
...
class ManualKeywordView(APIView):
    ...
    def get(self, request):
        try:
            suggestions = SuggestedInterest.objects.filter(is_active=True).order_by("display_order")
            data = {}
...
class DeleteKeywordView(APIView):
    ...
    def delete(self, request):
        ...
        try:
            ...
            # ✅ Profile.interests 동기화
            profile, _ = Profile.objects.get_or_create(user=user)
            remaining_ids = Interest.objects.filter(user=user).values_list('id', flat=True)
            profile.interests.set(remaining_ids)
            profile.save()
...
class SaveExtractedKeywordsView(APIView):
    ...
    def post(self, request):
        user = request.user
        data = request.data  # 이미 {"취미": [...], "음악": [...]} 형식

        try:
            for category_name, keywords in data.items():
                category, _ = InterestCategory.objects.get_or_create(name=category_name)
