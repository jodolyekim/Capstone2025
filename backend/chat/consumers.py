...
from .utils.message_restriction import is_sending_restricted  # ✅ 제재 로직 추가
...
class ChatConsumer(AsyncWebsocketConsumer):
    ...
    async def receive(self, text_data):
        ...
        try:
            ...
            # ✅ 송신 제한 검사
            is_blocked, block_reason = await sync_to_async(is_sending_restricted)(sender, chatroom_obj)
            if is_blocked:
                print("⛔ 제재 조건 감지됨:", block_reason)
                await self.send(text_data=json.dumps({
                    'input_msg': f"🚫 전송이 차단되었습니다: {block_reason}",
                    'sender': 'SYSTEM',
                    'type': 'text',
