...
def get_chat_room_detail(request, room_id):
    ...
    return Response({
        ...
        "other_user_name": getattr(getattr(other_user, 'profile', None), 'name', '이름 없음')
    })
...
def send_message_to_room(request):
    ...
    image_file = request.FILES.get("image")  # ✅ 이미지 파일 추출
    ...
    message = Message.objects.create(
        ...
        reason=reason or "",
        image=image_file if msg_type == "image" and image_file else None
    )

    # ✅ 이미지 전송 시 보호자 문자 전송
    if msg_type == "image":
        from sms.utils import notify_guardian_if_needed
        notify_guardian_if_needed(request.user, event_type="사진 전송", message=message)

    # ✅ Flutter에 경로 포함해서 전송
    return Response({
        "sender": str(request.user.email),
        "msg_type": msg_type,
        "filtered_msg": message.image.url if msg_type == "image" and message.image else message.filtered_msg
    }, status=201)
...
def report_chat(request):
    ...
    # ✅ 문자열 → UUID 객체 변환
    try:
        uuid_room_id = uuid.UUID(room_id)
    except ValueError:
        return Response({"error": "유효하지 않은 UUID 형식입니다."}, status=400)

    # ✅ UUID 필드(chatroom)로 채팅방 조회
    try:
        room = ChatRoom.objects.get(chatroom=uuid_room_id)
    except ChatRoom.DoesNotExist:
        return Response({"error": "채팅방이 존재하지 않습니다."}, status=404)
    ...
    # ✅ 중복 신고 방지 (UUID 필드 기준으로 정확히 비교)
    if ChatReport.objects.filter(reporter=user, chat_room__chatroom=uuid_room_id).exists():
        return Response({"error": "이미 이 채팅방을 신고하셨습니다."}, status=400)

    # ✅ 대상 사용자 찾기
    target_user = room.participants.exclude(id=user.id).first()

    # ✅ 최근 메시지 로그 추출 (최신순 20개)
