...
def gpt_filter_check(message: str) -> bool:
    """
    GPT API를 이용해 탐지된 문장이 실제 전송 불가한 민감 표현인지 판단
    True: 전송 가능, False: 전송 차단
    """
    response = openai.ChatCompletion.create(
        model="gpt-4",
        messages=[
            {"role": "system", "content": "다음 문장이 욕설, 비속어, 개인정보, 외부 SNS 공유 등 민감 표현인지 판단해주세요. 문제가 없으면 '정상', 문제가 있으면 '차단'으로만 답해주세요."},
...
def filter_message(message: str) -> tuple[bool, str, list]:
    """
    전체 필터링 로직
    1. 형식 DB 검사
    2. 일치하면 GPT로 추가 판단
    3. 최종 판단 반환
    """
    hits = check_predefined_patterns(message)
    if not hits:
        return True, "전송 가능", []

    try:
        is_safe = gpt_filter_check(message)
    except Exception:
        return False, "GPT 연동 오류: 메시지를 전송할 수 없습니다.", hits

    if is_safe:
        return True, "GPT 판단 결과 전송 가능", hits
    else:
        return False, "민감 표현 탐지: 전송 차단됨", hits


# 예시 테스트
if __name__ == "__main__":
    test_msg = "010-1234-5678 이거 내 번호야 카톡 알려줘"
    ok, reason, detected = filter_message(test_msg)
    print("전송 가능 여부:", ok)
    print("차단 사유:", reason)
