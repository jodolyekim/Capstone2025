...
def send_message_to_room(request):
    ...
    input_msg = request.data.get('input_msg', '').strip()
    ...
    input_msg=input_msg,
    msg_type=msg_type,
    format_filtered=bool(reason),  # reason이 있으면 True
    gpt_filtered=False,  # GPT는 여기선 호출 안 됐으니 False
    filtered_msg=input_msg,  # 현재는 원문 그대로 저장
    reason=reason or ""  # ❗ None 방지, NOT NULL 에러 방지
)
