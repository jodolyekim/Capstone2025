...
def get_user_chat_rooms(request):
    ...
    Match 모델 기반으로 user1 또는 user2 조건으로 조회합니다.
    """
    user = request.user
    rooms = ChatRoom.objects.filter(match__user1=user) | ChatRoom.objects.filter(match__user2=user)

    result = []
    for room in rooms.distinct():
        # 상대방 유저 식별
        target_user = room.match.user2 if room.match.user1 == user else room.match.user1
