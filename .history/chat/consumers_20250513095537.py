...
from .utils.message_filtering import check_predefined_patterns  # 🔥 형식 필터링 모듈
from .utils.gpt_judge import is_sensitive_message  # 🔥 GPT 판단 모듈
    ...
    async def receive(self, text_data):
        ...
        try:
            ...
            if detected:
                ...
                gpt_result = await sync_to_async(is_sensitive_message)(input_msg)
                print("🧠 GPT 판단 결과:", gpt_result)

                if gpt_result:  # GPT가 YES라고 판단
                    warning_msg = f"⚠️ 전송이 차단되었습니다: {reason} 포함됨 (GPT 판단)"
