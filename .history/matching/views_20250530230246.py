...
def get_candidates(request):
    ...
    my_gender = profile._gender
    my_orientation = profile._sex_orientation
    max_distance = profile._match_distance or 5

    # user.profile.interests로 변경
    ...
    for m in past_matches:
        if m.user1 == user and m.status_user1 != 'pending':
            exclude_ids.add(m.user2.id)
        elif m.user2 == user and m.status_user2 != 'pending':
            exclude_ids.add(m.user1.id)
    ...
    for candidate in candidates:
        ...
        # ✅ 성별/성적 지향 필터링
        if my_orientation == "이성을 만나고 싶어요":
            if my_gender == "남자" and candidate._gender != "여자":
                continue
            if my_gender == "여자" and candidate._gender != "남자":
                continue
            if candidate._sex_orientation not in ["이성을 만나고 싶어요", "모두 만나고 싶어요"]:
                continue
        elif my_orientation == "동성을 만나고 싶어요":
            if my_gender != candidate._gender:
                continue
            if candidate._sex_orientation not in ["동성을 만나고 싶어요", "모두 만나고 싶어요"]:
                continue
        elif my_orientation == "모두 만나고 싶어요":
            if candidate._sex_orientation not in [
                "이성을 만나고 싶어요", "동성을 만나고 싶어요", "모두 만나고 싶어요"
            ]:
                continue

        # ✅ 키워드 필터링 candidate_user.profile.interests 로 변경
        candidate_keywords = set(candidate.profile.interests.values_list('keyword', flat=True))
