...
from users.models import CustomUser, Profile  # ✅ 수정
from matching.models import Match, ChatRoom  # ✅ 수정
...
def get_candidates(request):
    ...
    for candidate in candidates:
        ...
        # 성별/성적 지향 조건
        if my_gender and candidate._gender and my_gender != candidate._gender:
            continue
        if my_orientation and candidate._sex_orientation and my_orientation != candidate._sex_orientation:
            continue

        # 사진 URL
        photo_url = None
        if hasattr(candidate_user, 'photos'):
            photo = candidate_user.photos.first()
            if photo:
                photo_url = request.build_absolute_uri(photo.photo_url)
