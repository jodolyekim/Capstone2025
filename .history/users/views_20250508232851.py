...
def update_profile_partial(request):
    print("🔥 [Django] PATCH 요청 들어옴")

    # 받은 토큰 디코딩 시도
    raw_token = request.headers.get('Authorization', '').split(' ')[1]
    print("📦 [Django] 받은 토큰:", raw_token)

    try:
        decoded = UntypedToken(raw_token)
        print("✅ [Django] 토큰 디코딩 성공:", decoded)
    except Exception as e:
        print("❌ [Django] 토큰 디코딩 실패:", str(e))

    user = request.user
    print("👤 [Django] request.user.email:", user.email)
    print("📥 [Django] 받은 데이터:", request.data)
