...
# Guardian 관리자 등록 (유저 이메일 - 보호자 이름 보기)
@admin.register(Guardian)
class GuardianAdmin(admin.ModelAdmin):
    list_display = ['guardian_display', 'relation', 'is_approved', 'is_visible']
    search_fields = ['user__email', 'name']
    list_filter = ['relation', 'is_approved', 'is_visible']

    def guardian_display(self, obj):
        return f"{obj.user.email} - {obj.name}"
    guardian_display.short_description = "유저 이메일 - 보호자 이름"
