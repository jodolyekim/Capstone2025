...
# ✅ 프로필 시리얼라이저 (interests: user 기준으로 조회)
class ProfileSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            'match_distance',
            'protector_info_name',
            'protector_info_birth_date',
            'protector_info_phone',
            'protector_info_relationship',
            'is_approved',
            'is_rejected',
            'interests',
        ]
        ...
        extra_kwargs = {
            ...
            'my_gender': {'required': False, 'allow_blank': True},
            'preferred_gender': {'required': False, 'allow_blank': True},
            'communication_way': {'required': False},
            'current_location_lat': {'required': False},
            'current_location_lon': {'required': False},
            'match_distance': {'required': False},
            'protector_info_name': {'required': False, 'allow_blank': True},
            'protector_info_birth_date': {'required': False},
            'protector_info_phone': {'required': False, 'allow_blank': True},
            'protector_info_relationship': {'required': False, 'allow_blank': True},
        }

    def get_interests(self, obj):
        return list(obj.user.interests.values_list('keyword', flat=True))
...
class CustomTokenObtainPairSerializer(TokenObtainPairSerializer):
    ...
    def validate(self, attrs):
        ...
        try:
            ...
            step = 0
            if all([profile.name, profile.birth_date, profile.my_gender, profile.preferred_gender]):
                step = 1
