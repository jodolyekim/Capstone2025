...
class Profile(models.Model):
    ...
    sexual_orientation = models.CharField(max_length=20, blank=True, null=True)

    # 커뮤니케이션 선호
    communication_styles = models.JSONField(default=list, blank=True)  # ✅ 리스트 저장용

    # 위치 및 매칭 거리
    latitude = models.FloatField(null=True, blank=True)
    longitude = models.FloatField(null=True, blank=True)
    match_distance_km = models.IntegerField(default=5, null=True, blank=True)

    # 보호자 기본 정보
    guardian_name = models.CharField(max_length=100, default='홍길동')
    guardian_birth_date = models.DateField(null=True, blank=True)
    guardian_phone = models.CharField(max_length=20, default='010-0000-0000')
    guardian_relationship = models.CharField(max_length=50, default='보호자')
    ...
    def is_complete(self):
        """
        프로필 설정이 완료되었는지 확인하는 함수
        설정이 완료된 경우 True, 아니면 False
        """
        return bool(self.name and self.birth_date and self.gender and self.sexual_orientation)
