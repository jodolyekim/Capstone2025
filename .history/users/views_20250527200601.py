...
class GuardianUploadView(APIView):
    ...
    def post(self, request):
        try:
            ...
            guardian.family_certificate_url = request.data.get("family_certificate_url", "")
            guardian.disability_certificate_url = request.data.get("disability_certificate_url", "")
            guardian.save()
            return Response({"message": "업로드 완료"}, status=200)
