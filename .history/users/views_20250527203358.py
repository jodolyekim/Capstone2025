...
def save_or_update_profile(request):
    ...
    if serializer.is_valid():
        ...
        # ✅ step == 5이면 is_profile_set 플래그 True로 변경
...
# ✅ 최종 완료 시 호출 (is_profile_set 플래그 설정)
    ...
    def post(self, request):
        try:
            ...
            if 'family_certificate' in request.FILES:
                ...
                family_path = default_storage.save(f"guardian/family_{request.user.id}_{family_file.name}", family_file)
            ...
            if 'disability_certificate' in request.FILES:
                ...
                disability_path = default_storage.save(f"guardian/disability_{request.user.id}_{disability_file.name}", disability_file)
