...
# 보호자 모델 (사용자와 다대일 관계)
cclass Guardian(models.Model):
    user = models.ForeignKey(CustomUser, on_delete=models.CASCADE, related_name='guardians')
    name = models.CharField(max_length=50)
    phone = models.CharField(max_length=20)
    relation = models.CharField(
        max_length=30,
        choices=[('아빠', '아빠'), ('엄마', '엄마'), ('형제자매', '형제자매')]
    )
    family_certificate = models.ImageField(upload_to='certificates/family/', null=True, blank=True)
    disability_certificate = models.ImageField(upload_to='certificates/disability/', null=True, blank=True)
