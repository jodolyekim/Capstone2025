...
class ProfileAdmin(admin.ModelAdmin):
    list_display = (
        'user', 'name', 'birth_date', 'gender', 'sexual_orientation',
        'pretty_communication_styles',  # ✅ 여기만 바꿔줌
        'latitude', 'longitude', 'match_distance_km'
    )
    search_fields = ('user__email', 'name')
    list_filter = ('gender', 'sexual_orientation')

    def pretty_communication_styles(self, obj):
        if not obj.communication_styles:
            return "-"
        return ", ".join(obj.communication_styles)
    pretty_communication_styles.short_description = "소통 스타일"
